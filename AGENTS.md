...
- Test the kit with an OpenCode sandbox (via PowerShell on Windows); Template-Version **gepinnt** auf `0.6.0`:
  ```powershell
  sbx run opencode `
      --kit ./opencode-agent/ `
      ...
      --static-mcp idea
  ```
- Test the kit with a Claude Code sandbox (via PowerShell on Windows); Template-Pin `0.6.0` (Home, `api.anthropic.com`):
  ```powershell
  sbx run claude `
      --kit ./opencode-agent/ `
      ...
      --static-mcp idea
  ```
- Run the dedicated Mammouth agent kit (kind: sandbox, entrypoint `mammouth`); Template-Pin `0.6.0` steckt im spec-Image (kein `--template` nötig):
  ```powershell
...
- `mistral-vibe-agent/Dockerfile` — pinned Vibe image (shell-docker-0.6.0 + `uv tool install mistral-vibe==<pin>`); Build/Publish via `.github/workflows/publish-mistral-vibe-image.yml`
...
> **Mistral Vibe**: Installiert das Agent-Kit als eigenes gepinntes Image (`mistral-vibe-agent/Dockerfile`: shell-docker-0.6.0 + `uv tool install mistral-vibe==<pin>`, Renovate `mistral-vibe` PyPI). API-Key via Built-in-Service `mistral` (`MISTRAL_API_KEY`, `api.mistral.ai`, Sentinel `proxy-managed`). Launch via `sandbox.entrypoint: [vibe, "--agent", "auto-approve"]` in der spec (ohne entrypoint startet sbx die Default-Shell). Image muss vor dem ersten Start publiziert sein (`publish-mistral-vibe-image.yml`, workflow_dispatch).
