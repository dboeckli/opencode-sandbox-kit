...
The IntelliJ MCP server is connected via `host.docker.internal:64342/sse`. Tools are exposed as MCP tools (e.g. `idea_search_symbol`, `idea_read_file`, `idea_build_project`, `idea_get_file_problems`, `idea_execute_sql_query`, debugger tools). The full set of available tools is exposed to the agent automatically by the MCP server — they appear in the tool list at the start of every session. Use them to interact with the IntelliJ IDE on the Windows host: navigate code, run inspections, build, debug, and query the database. These tools require IntelliJ IDEA to be running on the host with the MCP server plugin enabled.
...
Before making an outbound request (`curl`, `npm`, `git clone`, web search, ...), check this list.
Prefer whitelisted endpoints: `npx ctx7 docs` for library docs, `gh` / `api.github.com` for GitHub,
`npm` against `registry.npmjs.org`, `docker pull` against `docker.io`.
