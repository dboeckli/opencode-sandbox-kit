...
The **Docker / Kubernetes** sidebar box runs `~/.config/sandbox-kit/check-infra.sh` (live from the TUI, every 10s). It probes real reachability, not just CLI presence: `docker info` (daemon in the sandbox microVM), `docker -H tcp://host.docker.internal:2375 info` (optional Docker Desktop host daemon) and `kubectl get --request-timeout=3s nodes` (cluster via the `~/.kube/config` auto-generated by `setup.startup`). Output format: `docker:OK docker-host:OK kubernetes:OK` (or `FAIL` per component). Both probes are bounded by `timeout`, so the box never hangs. Self-healing: if `~/.kube/config` is missing (e.g. the `.kube` mount raced `setup.startup`), the check regenerates it on the fly before probing. Manual run:
