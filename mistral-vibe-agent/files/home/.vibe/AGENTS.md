...
<!-- sandbox-tools -->
This sandbox is provisioned by the mistral-vibe-agent kit. The following tools are installed and available:

## Mistral Vibe docs

Mistral Vibe is available in Context7 (`npx ctx7 docs /mistralai/mistral-vibe <query>`) and on GitHub
(https://github.com/mistralai/mistral-vibe). Configuration lives in `~/.vibe/config.toml` (or `./.vibe/config.toml`);
...
the host (`sbx mcp add idea --url http://localhost:64615/stream --skip-ssrf-check`) and create the sandbox with
`--static-mcp idea` (or `sbx mcp load idea --sandbox`). Tools arrive through the gateway as `mcp-gateway_<tool>`
(e.g. `mcp-gateway_get_symbol_info`, `mcp-gateway_search_symbol`, `mcp-gateway_analyze_calls`).

The gateway is wired into Vibe via `~/.vibe/config.toml` (a `[[mcp_servers]]` entry, `transport = "streamable-http"`,
`url = "http://mcp-gateway.docker.internal/mcp"`, static auth `Authorization: Bearer proxy-managed`). Because the
...
**sbx CLI** (auf dem Host via PowerShell): Offline-Referenz aller `--help`-Outputs unter `~/sbx-cli.md` — sie liegt im Kit-Bundle (`files/home/sbx-cli.md`, v0.43.0).
...
If a check reports FAIL, mention it and suggest a fix. Details: `~/.config/sandbox-kit/startup-checks.md`.
<!-- sandbox-tools -->
